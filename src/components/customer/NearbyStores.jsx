    ...
    <section className="bg-cream py-12 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-2xl font-bold text-olive">Stores Near You</h2>
            <p className="text-olive/50 text-sm mt-1 flex items-center gap-1">
              <MapPin size={13} /> Based on your location — Lahore
            </p>
          </div>
          <Link
            to="/stores"
        ...
        {/* Stores — horizontal scroll mobile, grid desktop */}
        <div className="flex gap-4 overflow-x-auto pb-2 md:grid md:grid-cols-3 lg:grid-cols-5 md:overflow-visible scrollbar-hide">
          {stores.map((store) => (
            <Link
              key={store.id}
              to={`/store/${store.id}`}
              className="min-w-[220px] md:min-w-0 bg-white rounded-2xl p-4 border border-olive/10 hover:shadow-md hover:border-purple/30 transition-all duration-200 flex-shrink-0 group"
            >
              {/* Store Icon */}
              ...
              {/* Name */}
              <h3 className="text-sm font-bold text-olive mt-2 leading-tight group-hover:text-purple transition-colors">
                {store.name}
              </h3>
